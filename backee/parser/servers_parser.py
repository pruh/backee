...
def __parse_ssh_server(
    server: Dict[str, Any], rotation_strategy: RotationStrategy
) -> BackupServer:
...
def __parse_server(
    server: Dict[str, Any], default_rs: RotationStrategy
) -> BackupServer:
    supported_servers = {"ssh": __parse_ssh_server}
    ...
    rotation_strategy = (
        parse_rotation_strategy(server["rotation_strategy"])
...
def parse_servers(
    servers: Tuple[Dict[str, Any]], default_rs: RotationStrategy
) -> Tuple[BackupServer]:
    if servers is None:
        return ((),)
